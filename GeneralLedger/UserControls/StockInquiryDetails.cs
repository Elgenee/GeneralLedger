...
{
    public partial class StockInquiryDetails :  MetroForm
    {

        public GeneralLedger.Tier.BO.Product Product { get; set; }
        ...
        private void StockInquiryDetails_Load(object sender, EventArgs e)
        {
            ProductBAL productBAL = new ProductBAL();
            var stockDetailsList = productBAL.getStockDetailsByProductId(this.ProductId);

            var totalProductIn = stockDetailsList.Sum(s => s.QuantityIn);
            this.txtTotalInAmount.Text = totalProductIn.ToString();

            var totalProductOut = stockDetailsList.Sum(s => s.QuantityOut);
            this.txtTotalOutAmount.Text = totalProductOut.ToString();

            var totalRemainingCount = totalProductIn - totalProductOut;
            ...
            if ((stockDetailsList != null) && stockDetailsList.Count > 0)
            {
                ...
                {
                    StockDetailsByProductId stockDetailsByProductId = stockDetailsList[i];
